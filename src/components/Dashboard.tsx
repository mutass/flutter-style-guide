...
const navItems = [
  ...
  { id: "email", icon: Mail, label: "CRM & Comms" },
  { id: "pages", icon: Globe, label: "Landing Pages" },
  { id: "pdf", icon: FileText, label: "PDF Engine" },
  { id: "agent", icon: Headphones, label: "AI Agent" },
];

const metrics = [
  { label: "Total Leads", value: "248", sub: "+18 this week", trend: "up" },
  { label: "Calls Made", value: "1,847", sub: "+62% answer rate", trend: "up" },
...
const sourceBadge = (source: string) => {
  const map: Record<string, string> = {
    "Nomad Page": "bg-primary/10 text-primary",
    "Chapter Teaser": "bg-[rgba(255,255,255,0.06)] text-muted",
    "Pre-order": "bg-gold/10 text-gold",
  };
  ...
  return (
    <div className="h-screen w-full flex bg-background">
      ...
      <aside className="w-[248px] flex-shrink-0 border-r border-[rgba(255,255,255,0.06)] flex flex-col p-6 px-3.5" style={{ background: "hsl(222 50% 6%)" }}>
        ...
        <div className="text-[0.62rem] font-bold uppercase tracking-[0.12em] text-muted px-2.5 mt-4 mb-1.5">Core</div>
        ...
        <div className="text-[0.62rem] font-bold uppercase tracking-[0.12em] text-muted px-2.5 mt-4 mb-1.5">Tools</div>
        ...
        <div className="mt-auto bg-emerald/[0.06] border border-emerald/[0.15] rounded-lg p-3">
          <div className="text-[0.62rem] font-bold uppercase tracking-[0.1em] text-muted mb-1">System Status</div>
          <div className="flex items-center gap-1.5 text-xs font-semibold text-emerald">
            <div className="w-1.5 h-1.5 rounded-full bg-emerald shadow-[0_0_8px_hsl(var(--emerald))]" />
            All Systems Online
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-background">
        ...
        <div className="flex items-center justify-between px-8 py-4 border-b border-[rgba(255,255,255,0.06)] flex-shrink-0">
          <div>
            <div className="font-extrabold text-xl tracking-tight text-foreground capitalize">{activeTab === "dashboard" ? "Command Center" : navItems.find(n => n.id === activeTab)?.label}</div>
            <div className="text-xs text-muted mt-0.5">Welcome back, Author</div>
          </div>
          <div className="flex items-center gap-2.5">
            <div className="flex items-center gap-2 bg-card border border-[rgba(255,255,255,0.06)] rounded-full px-3.5 py-1.5 cursor-pointer transition-all hover:border-primary/30">
              ...
              <div>
                <div className="text-xs font-semibold text-foreground">Sarah AI</div>
                <div className="text-[0.62rem] font-bold text-emerald">Online</div>
              </div>
            </div>
            <button onClick={onLogout} className="bg-primary/10 border border-primary/20 text-primary px-4 py-2 rounded-lg font-semibold text-xs transition-all hover:bg-primary hover:text-primary-foreground flex items-center gap-1.5">
              <LogOut size={14} /> Exit
            </button>
          </div>
        </div>
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {metrics.map((m, i) => (
          <div key={i} className="bg-card border border-[rgba(255,255,255,0.06)] rounded-[13px] p-5 transition-all hover:border-primary/20 hover:-translate-y-0.5">
                ...
                <tr key={i} className="border-t border-[rgba(255,255,255,0.06)] transition-colors hover:bg-primary/[0.03]">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div className={`w-9 h-9 rounded-full bg-gradient-to-br ${lead.color} flex items-center justify-center font-extrabold text-xs text-primary-foreground flex-shrink-0`}>
                        {lead.initials}
                      </div>
                      <div>
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[0.67rem] font-semibold ${sourceBadge(lead.source)}`}>{lead.source}</span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[0.67rem] font-semibold bg-emerald/10 text-emerald">✓ {lead.consent}</span>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[0.67rem] font-semibold ${stageBadge(lead.stage)}`}>{lead.stage}</span>
                  </td>
...
function getTabDesc(id: string) {
  const map: Record<string, string> = {
    ...
    pages: "Build high-conversion landing pages with live preview and templates.",
    pdf: "Upload DOCX or PDF, format to KDP standards, and export print-ready files.",
    agent: "Configure Sarah AI — your voice agent for automated lead follow-ups.",
  };
  return map[id] || "";
}
